...
class CacheBase:
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.cache_dir, self.url)) as fp:
            fp.write(self.body)
...
class TestCacheText(testtools.TestCase, CacheBase):
    text_or_binary = 'text'

class TestCacheBinary(testtools.TestCase, CacheBase):
